...
describe("logic", () => {
  ...
  beforeEach(() => {
    return Promise.all([User.remove(), Event.remove(), Company.remove()])
  })
  ...
  describe("validate ticket", () => {
    ...
    before(() => {
      ...
      return Promise.all([event.save().then(_event => (event = _event))])
        .then(() => {
    ...
    it("should validate ticket", () => {
      ...
      assert.equal(
        _session._id.toString(),
        session._id.toString(),
        "should session match"
      )
