...
const logic = {
    ...
    return new Promise((resolve, reject) => {
      User.find({ _id: userId })
        ...
        .then(user => {
          Event.populate(user, {
            path: "events",
            select: "company image subtitle title"
          }).then(resolve)
        })
        .catch(reject)
    })
  ...
  getTickets(eventId, sessionId) {
    return new Promise((resolve, reject) => {
      Event.findOne({ _id: eventId })
        .select({ title: 1, "sessions.date": 1, "sessions.location": 1 })
        .then(resolve)
        .catch(reject)
    })
  },
    ...
    return Event.findOne({ _id: eventId })
      .then(event => {
        ...
        event.sessions
          .id(sessionId)
          .tickets.id(ticketId)
          .set({ status: true, validated: Date.now() })
        return event.save()
      })
  ...
  getCompanyByUsers(companyId) {
    return new Promise((resolve, reject) => {
      return User.find({ companies: companyId, role: "validator" })
      ...
      return User.findOne({ email: email })
        .then(user => {
          ...
          if (user.email == emailUser)
            throw Error("You cannot delete your admin user")
          if (role.toLowerCase() != "admin")
            throw Error("You cannot delete an admin user")
          if (user.email !== email)
            throw Error("User email does not match the one provided")
